...
import { advance, initialState, restore, clues, questions, rooms, witnesses, requiredWitnesses, confrontations, cast, envelopes, mainRule, photos, deskNote, livingTimeline, accusationVerdict, blackout, type Action } from '@/lib/case';
...
export default function Home() {
  ...
  const [evidencePicks, setEvidencePicks] = useState<number[]>([]);
    ...
    <main className="game-shell">
      ...
      <div className="workspace">
        ...
        <section className="case-content">
          ...
          {view === 'conclusion' && confronted && <div className="map-view">
            ...
            {!game.reconstruction ? <article className="brief-card deduction-card"><h2>¿Qué relato explica mejor el conjunto?</h2><div className="question-list">
              ...
              {!game.solved ? <article className="brief-card accusation"><span>Acusación final</span><h2>Presenta una teoría completa</h2>
                ...
                <fieldset><legend>¿Qué pruebas sostienen tu teoría?</legend><p className="field-hint">Elige sólo las que demuestren el motivo, el acceso y el momento. Una prueba que no se puede atribuir a nadie debilita la acusación.</p><div className="evidence-picks">{clues.map(clue => { const id = `evidence-${clue.id}`; const checked = evidencePicks.includes(clue.id); return <label className="radio-option" htmlFor={id} key={clue.id}><input type="checkbox" id={id} checked={checked} onChange={() => setEvidencePicks(current => checked ? current.filter(value => value !== clue.id) : [...current, clue.id].sort((a, b) => a - b))} /><span>{clue.title}<small>{clue.location}</small></span></label>; })}</div></fieldset>
                <Button className="primary-action" onClick={() => { const verdict = accusationVerdict({ accused, event: eventTheory, exit: exitTheory, evidence: evidencePicks }); if (verdict.ok) dispatch({ type: 'solve' }); note(verdict.message, verdict.tone); }}>Confirmar acusación</Button><Feedback {...feedback} />
          ...
          {view === 'cronologia' && (() => { const rows = livingTimeline(game); const linked = rows.filter(row => row.notes.length > 0).length; const kindLabel = { prueba: 'Prueba', declaracion: 'Declaración', deduccion: 'Deducción', ficha: 'Ficha' }; return <div className="map-view"><Envelope n={2} /><h1>Los minutos del apagón</h1><p className="lead">Horarios recogidos en el expediente inicial. Cada momento se ilumina cuando una prueba, una declaración o una deducción lo explica.</p>
            <p className="timeline-count">{linked} de {rows.length} momentos relacionados · la franja roja marca el apagón</p>
            <ol className="timeline-list living">{rows.map(row => <li key={row.time} className={`${row.blackout ? 'blackout' : ''} ${row.notes.length > 0 ? 'linked' : ''} ${row.derived ? 'derived' : ''}`.trim()}><time>{row.time}</time><div>{row.event}{row.time === blackout.start && <em className="blackout-tag">Comienza el apagón</em>}{row.time === blackout.end && <em className="blackout-tag">Termina el apagón</em>}{row.derived && <em className="blackout-tag derived-tag">Hora añadida por una prueba, no por un testigo</em>}{row.notes.length > 0 && <ul className="timeline-notes">{row.notes.map(item => <li key={item.text} className={item.kind}><b>{kindLabel[item.kind]}</b>{item.text}</li>)}</ul>}</div></li>)}</ol></div>; })()}
          </>}
        </section>
      </div>
